...
Sim::OPTS manages parametric explorations through the ESP-r building simulation platform and performs multiobjective optimization by block coordinate descent.
...
The OPTS configuration file has to contain a suitable description of the operations to be accomplished.  A great deal of instructions which are necessary to run OPTS will usually be specified in that file, which has to point to an existing ESP-r model.  In this distribution there is a template file with explanations and an example of OPTS configuration file.
...
Besides an OPTS configuration file, also configuration files for propagation of constraints may be prepared.  This can give to the morphing operations much greater flexibility.  Propagation of constraints can regard the geometry of a model, solar shadings, mass/flow network, and/or controls; and also, how those pieces of information affect each other and daylighting (calculated through the Radiance lighting simulation program).  To get some hints about what propagation on constraints can be used for in the program, an example file included in this distribution may be studied.
...
1) If brute force optimization is sought for a case composed by 4 parameters, the following settings should be specified: <@varn = (1, 2, 3, 4);> and <@casegroup = ( [ [4, 4] ] ) ;>.

b) If a block search is sought on the basis of 3 parameters, with 2 overlapping active blocks composed by 2 parameters each having the leftmost parameters in position 3 and 4 of the mentioned joined list, the following settings should be specified: <@varn = ( [ [1, 2, 3] ] );> and <@casegroup = ( [ [3, 2], [4, 2] ] );>.
