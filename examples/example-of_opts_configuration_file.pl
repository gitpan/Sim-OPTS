...
$mypath= "/home/ubuntu"; # Write here the path of the directory in which you are going to work
$homepath= "/home/ubuntu";
$fileconfig=("cellw.cfg"); # Write here the name of the configuration file of you model


$outfile = "$mypath/$file-$fileconfig.feedback.txt"; # Write here the name of the files in which reports will be printed in order to check the inner working of the program.  In order to do this, you'll have to mess up with the code to specify what you want to be printed.
$toshell = "$mypath/$file-$fileconfig.feedbacktoshell.txt"; # Write here the name of the files in which the output to the shell will be printed in order to check the inner working of the program.  In order to do this, you'll have to mess up with the code to specify what you want to be printed.
...
@themereports = ("loads"); # possibilities: "loads", "temperatures" ######### General themes regarding the simulation.  This piece of information go to compose the result name files.
@simtitle = ("feb01-feb28"); # write here the name of the time periods will be taken into account in the simulations.
@reportperiods = ("feb01-feb28", "feb01-feb28"); # write here the name of the time periods will be taken into account in the reports.
...
@retrievedataloads = ("1 2 1", "28 2 24", "1"); # start data to retrieve for loads; end data to retrieve for loads.
@retrievedatatempsstats = ("1 2 1", "28 2 24", "1"); # start data to retrieve for temps; end data to retrieve for temps.

@rankdata = ("n", "n", "y", "n"); # THIS DATA ARE POINT-TO- POINT RELATIVE TO THE ARRAY ABOVE AND TELL WHAT FIELD THE RANKING HAS TO BE BASED ON 
...
$reportloadsdata = [[$simtitle[0], $simtitle[1]], "zone", ["1"], "zone"]; # mixdata y/n,  # "name of the files you want to deal with, if they are not all the existing ones", # list of the the names of the simdata to you want to mix together
